...
def run_smart(star, planet, ptfile, wlmin, wlmax, R=100000., rm_when_done=False,
        ...
        place = '../data/smart_outputs/%s/%s/%s/' % (star, planet, ptfile.split('.')[0])

        ptfile = PT_DIR + ptfile
        try:
            os.mkdir(place)
        except OSError:
            pass
        ...
        if star == 'trappist1':
            ...
            sim.smartin.spec = '/Users/Jake/Projects/Packages/photochem_smart/fixed_input/specs/Kurucz1cm-1_susim_atlas2.dat'
            sim.smartin.spec_skip=12
            sim.smartin.spec_unit = 1
            sim.smartin.radstar = 1.0

        if planet == 'earth':
            sim.smartin.r_AU = 1.0
            sim.smartin.radius = 6371.
            sim.smartin.grav = 9.8
